document.addEventListener('DOMContentLoaded', function() {
  ...
  const formToShow =
      document.getElementById(`${toCamelCase(content.type)}Form`);
  if (formToShow) {
    formToShow.style.display = 'block';
  }

  // Automatically set the current time for the start_date field if empty
  const startDateField = formToShow.querySelector('input[name="start_date"]');
    ...
    const formattedNow =
        `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${
            ...
            String(now.getMinutes())
                .padStart(2, '0')}`;  // Format as "YYYY-MM-DDTHH:mm"
    startDateField.value = formattedNow;
  }

  // Set the end_date field only if it has a valid value or leave it empty
  const endDateField = formToShow.querySelector('input[name="end_date"]');
  if (endDateField && !endDateField.value && content.end_date) {
    const endDate = new Date(content.end_date);
    const formattedEndDate = `${endDate.getFullYear()}-${
        String(endDate.getMonth() + 1).padStart(2, '0')}-${
        String(endDate.getDate()).padStart(2, '0')}T${
        String(endDate.getHours()).padStart(2, '0')}:${
        String(endDate.getMinutes())
            .padStart(2, '0')}`;  // Format as "YYYY-MM-DDTHH:mm"
  ...
  if (content.type === 'ProgramContent') {
    ...
    programData.time.forEach((time, index) => {
      ...
      newRow.innerHTML = `
                <td><input type="time" class="form-control" name="time" value="${
          time}" required></td>
                <td><input type="text" class="form-control" name="activity" value="${
          programData.activity[index]}" required></td>
                <td><input type="text" class="form-control" name="location" value="${
          programData.location[index]}"></td>
                <td><input type="text" class="form-control" name="notes" value="${
          programData.notes[index]}"></td>
  ...
  if (content.type === 'BirthdayContent') {
    ...
    birthdayData.birthday.forEach((birthday, index) => {
      const newRow = document.createElement('tr');
      newRow.innerHTML = `
                <td><input type="date" class="form-control" name="birthday" value="${
          birthday}" required></td>
                <td><input type="text" class="form-control" name="name" value="${
          birthdayData.name[index]}" required></td>
                <td>
                    <input type="file" class="form-control" name="image" accept="image/*">
                    <small class="form-text text-muted truncate">
                        ${
          birthdayData.image[index] ?
              `Aktuelle Datei: ${birthdayData.image[index]}` :
              ''}
                    </small>
                </td>
                <td><button type="button" class="btn btn-danger removeRowButton">Entfernen</button></td>
  ...
  document.getElementById('addBirthdayRowButton')
      .addEventListener('click', function() {
        const tableBody = document.getElementById('birthdayDetailsTable')
                              .querySelector('tbody');
        const newRow = document.createElement('tr');
        newRow.innerHTML = `
  ...
  document.getElementById('saveButton').addEventListener('click', function() {
    ...
    const startDate =
        formToShow.querySelector('input[name="start_date"]').value;
    const endDate = formToShow.querySelector('input[name="end_date"]').value;
    ...
    formToShow.querySelectorAll('input[type="file"]').forEach(input => {
      if (input.files.length > 0) {
        ...
        for (let i = 0; i < input.files.length; i++) {
          ...
          if (content.type === 'SlideshowContent' &&
          ...
          if (birthdayValue || nameValue) {
            ...
            if (imageInput.files.length > 0) {
              ...
              if (smallText && smallText.classList.contains('form-text')) {
                const currentFileText = smallText.textContent.trim().replace(
                    'Aktuelle Datei: ', '');
                birthdayData.image.push(currentFileText);
                existingFiles.push(currentFileText);
              } else {
                const existingFile =
                    content.content.birthdayTable.image[rowIndex];
    ...
    if (filesChanged) {
      formToShow.querySelectorAll('input[type="file"]')
          .forEach((input, index) => {
